...
public final class zzakv {
    public static X509Certificate[][] zza(String s) {
        ByteBuffer byteBuffer4;
        int v11;
        int v10;
        RandomAccessFile randomAccessFile0 = new RandomAccessFile(s, "r");
        try {
            Pair pair0 = zzakw.zzc(randomAccessFile0);
            if(pair0 != null) {
                ...
                if(v1 - 20L >= 0L) {
                    ...
                    if(randomAccessFile0.readInt() == 0x504B0607) {
                        throw new zzaks("ZIP64 APK not supported");
                    }
                }
                long v2 = zzakw.zza(byteBuffer0);
                if(v2 >= v1) {
                    throw new zzaks("ZIP Central Directory offset out of range: " + v2 + ". ZIP End of Central Directory offset: " + v1);
                }
                if(zzakw.zzb(byteBuffer0) + v2 != v1) {
                    throw new zzaks("ZIP Central Directory is not immediately followed by End of Central Directory");
                }
                if(v2 < 0x20L) {
                    throw new zzaks("APK too small for APK Signing Block. ZIP Central Directory offset: " + v2);
                }
                ...
                if(byteBuffer1.getLong(8) != 0x20676953204B5041L || byteBuffer1.getLong(16) != 0x3234206B636F6C42L) {
                    throw new zzaks("No APK Signing Block before ZIP Central Directory");
                }
                int v3 = 0;
                long v4 = byteBuffer1.getLong(0);
                if(v4 < ((long)byteBuffer1.capacity()) || v4 > 0x7FFFFFF7L) {
                    throw new zzaks("APK Signing Block size out of range: " + v4);
                }
                long v5 = v2 - ((long)(((int)(v4 + 8L))));
                if(v5 < 0L) {
                    throw new zzaks("APK Signing Block offset out of range: " + v5);
                }
                ...
                if(v6 != v4) {
                    throw new zzaks("APK Signing Block sizes in header and footer do not match: " + v6 + " vs " + v4);
                }
                ...
                while(byteBuffer4.hasRemaining()) {
                    ...
                    if(byteBuffer4.remaining() < 8) {
                        throw new zzaks("Insufficient data to read size of APK Signing Block entry #" + v3);
                    }
                    long v12 = byteBuffer4.getLong();
                    if(Long.compare(v12, 4L) < 0 || v12 > 0x7FFFFFFFL) {
                        throw new zzaks("APK Signing Block entry #" + v3 + " size out of range: " + v12);
                    }
                    int v13 = byteBuffer4.position();
                    if(((int)v12) > byteBuffer4.remaining()) {
                        throw new zzaks("APK Signing Block entry #" + v3 + " size out of range: " + ((int)v12) + ", available: " + byteBuffer4.remaining());
                    }
                    if(byteBuffer4.getInt() == 0x7109871A) {
                        zzakr zzakr0 = new zzakr(zzakv.zze(byteBuffer4, ((int)v12) - 4), v7, v2, v1, byteBuffer0, null);
                        X509Certificate[][] arr2_x509Certificate = zzakv.zzl(randomAccessFile0.getChannel(), zzakr0);
                ...
                throw new zzaks("No APK Signature Scheme v2 block in APK Signing Block");
            }
        }
        finally {
            randomAccessFile0.close();
        }
        throw new zzaks("Not an APK file: ZIP End of Central Directory record not found in file with " + randomAccessFile0.length() + " bytes");
    }
    ...
    private static ByteBuffer zze(ByteBuffer byteBuffer0, int v) {
    ...
    private static ByteBuffer zzf(ByteBuffer byteBuffer0) {
        ...
        return zzakv.zze(byteBuffer0, v);
    }
    ...
    private static void zzh(Map map0, FileChannel fileChannel0, long v, long v1, long v2, ByteBuffer byteBuffer0) {
        if(map0.isEmpty()) {
            throw new SecurityException("No digests provided");
        }
        zzakp zzakp0 = new zzakp(fileChannel0, 0L, v);
        zzakp zzakp1 = new zzakp(fileChannel0, v1, v2 - v1);
        ByteBuffer byteBuffer1 = byteBuffer0.duplicate();
        byteBuffer1.order(ByteOrder.LITTLE_ENDIAN);
        zzakw.zzd(byteBuffer1, v);
        zzakn zzakn0 = new zzakn(byteBuffer1);
        int v4 = map0.size();
        ...
        try {
            byte[][] arr2_b = zzakv.zzk(arr_v, new zzako[]{zzakp0, zzakp1, zzakn0});
        }
        ...
        for(int v3 = 0; v3 < v4; ++v3) {
            ...
            if(!MessageDigest.isEqual(((byte[])map0.get(v6)), arr2_b[v3])) {
                throw new SecurityException(zzakv.zzd(v6) + " digest of contents did not verify");
            }
        }
    }

    private static byte[] zzi(ByteBuffer byteBuffer0) {
        int v = byteBuffer0.getInt();
        if(v < 0) {
    ...
    private static X509Certificate[] zzj(ByteBuffer byteBuffer0, Map map0, CertificateFactory certificateFactory0) {
        X509Certificate x509Certificate0;
        Pair pair0;
        String s;
        ByteBuffer byteBuffer1 = zzakv.zzf(byteBuffer0);
        ByteBuffer byteBuffer2 = zzakv.zzf(byteBuffer0);
        byte[] arr_b = zzakv.zzi(byteBuffer0);
        ...
        while(byteBuffer2.hasRemaining()) {
            try {
                ++v1;
                ByteBuffer byteBuffer3 = zzakv.zzf(byteBuffer2);
                if(byteBuffer3.remaining() < 8) {
                    throw new SecurityException("Signature record too short");
                }
                int v2 = byteBuffer3.getInt();
                arrayList0.add(v2);
                if(v2 != 0x201 && v2 != 0x202 && v2 != 0x301 && (v2 != 0x101 && v2 != 0x102 && v2 != 0x103 && v2 != 260) || v != -1 && (zzakv.zzc(v2) == 1 || zzakv.zzc(v) != 1)) {
                    continue;
                }
                arr_b2 = zzakv.zzi(byteBuffer3);
                v = v2;
                continue;
            }
        ...
        byteBuffer1.clear();
        ByteBuffer byteBuffer4 = zzakv.zzf(byteBuffer1);
        ArrayList arrayList1 = new ArrayList();
        int v3 = 0;
        while(byteBuffer4.hasRemaining()) {
            try {
                ++v3;
                ByteBuffer byteBuffer5 = zzakv.zzf(byteBuffer4);
        ...
        if(arr_b3 != null && !MessageDigest.isEqual(arr_b3, arr_b1)) {
            throw new SecurityException(zzakv.zzd(v5) + " contents digest does not match the digest specified by a preceding signer");
        }
        ByteBuffer byteBuffer6 = zzakv.zzf(byteBuffer1);
        ArrayList arrayList2 = new ArrayList();
        int v6 = 0;
        while(byteBuffer6.hasRemaining()) {
            ++v6;
            byte[] arr_b4 = zzakv.zzi(byteBuffer6);
            try {
            ...
            arrayList2.add(new zzakt(x509Certificate0, arr_b4));
        }
    ...
    private static byte[][] zzk(int[] arr_v, zzako[] arr_zzako) {
        MessageDigest messageDigest1;
        int v11;
        int v = 0;
        long v1 = 0L;
        for(int v2 = 0; v2 < 3; ++v2) {
            v1 += (arr_zzako[v2].zza() + 0xFFFFFL) / 0x100000L;
        }
        if(v1 >= 0x1FFFFFL) {
        ...
        for(int v3 = 0; v3 < arr_v.length; ++v3) {
            byte[] arr_b = new byte[zzakv.zzb(arr_v[v3]) * ((int)v1) + 5];
            arr_b[0] = 90;
            zzakv.zzg(((int)v1), arr_b, 1);
            arr2_b[v3] = arr_b;
        }
    ...
    private static X509Certificate[][] zzl(FileChannel fileChannel0, zzakr zzakr0) {
        int v;
        ...
        try {
            byteBuffer0 = zzakv.zzf(zzakr0.zza);
            v = 0;
        }
        catch(IOException iOException0) {
        ...
        while(byteBuffer0.hasRemaining()) {
            try {
                ++v;
                arrayList0.add(zzakv.zzj(zzakv.zzf(byteBuffer0), hashMap0, certificateFactory0));
            }
        ...
        zzakv.zzh(hashMap0, fileChannel0, zzakr0.zzb, zzakr0.zzc, zzakr0.zzd, zzakr0.zze);
