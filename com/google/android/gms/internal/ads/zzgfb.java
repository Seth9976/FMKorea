...
abstract class zzgfb {
    ...
    public zzgfb(byte[] arr_b, int v) {
        if(arr_b.length != 0x20) {
            throw new InvalidKeyException("The key length in bytes must be 32.");
        }
        this.zza = zzgex.zzd(arr_b);
        this.zzb = v;
    }
    ...
    final ByteBuffer zzc(byte[] arr_b, int v) {
        int[] arr_v = this.zzb(zzgex.zzd(arr_b), v);
    ...
    public final byte[] zzd(byte[] arr_b, ByteBuffer byteBuffer0) {
        ...
        for(int v2 = 0; v2 < v1 / 0x40 + 1; ++v2) {
            ...
            if(v2 == v1 / 0x40) {
                zzgom.zza(byteBuffer1, byteBuffer0, byteBuffer2, v1 % 0x40);
            }
            else {
                zzgom.zza(byteBuffer1, byteBuffer0, byteBuffer2, 0x40);
            }
        }
