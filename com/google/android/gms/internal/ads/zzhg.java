...
public final class zzhg {
    ...
    static {
        zzhg.zza = Pattern.compile("bytes (\\d+)-(\\d+)/(?:\\d+|\\*)");
        zzhg.zzb = Pattern.compile("bytes (?:(?:\\d+-\\d+)|\\*)/(\\d+)");
    }
    ...
    public static long zza(String s, String s1) {
        ...
        if(!TextUtils.isEmpty(s)) {
            try {
                ...
                zzes.zzc("HttpUtil", "Unexpected Content-Length [" + s + "]");
            }
        }
        if(!TextUtils.isEmpty(s1)) {
            Matcher matcher0 = zzhg.zza.matcher(s1);
            if(matcher0.matches()) {
                try {
                    ...
                    if(v != v2) {
                        zzes.zzf("HttpUtil", "Inconsistent headers [" + s + "] [" + s1 + "]");
                        return Math.max(v, v2);
                    }
                }
                catch(NumberFormatException unused_ex) {
                    zzes.zzc("HttpUtil", "Unexpected Content-Range [" + s1 + "]");
                }
            }
        }
        return v;
    ...
    public static long zzb(String s) {
        ...
        Matcher matcher0 = zzhg.zzb.matcher(s);
        if(matcher0.matches()) {
