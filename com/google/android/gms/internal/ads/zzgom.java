...
public final class zzgom {
    ...
    public static byte[] zzb(byte[][] arr2_b) {
        int v1 = 0;
    ...
    public static final byte[] zzc(byte[] arr_b, byte[] arr_b1) {
        if(arr_b.length != arr_b1.length) {
            throw new IllegalArgumentException("The lengths of x and y should match.");
        }
        return zzgom.zzd(arr_b, 0, arr_b1, 0, arr_b.length);
    }

    public static final byte[] zzd(byte[] arr_b, int v, byte[] arr_b1, int v1, int v2) {
        if(arr_b.length - v2 < v || arr_b1.length - v2 < v1) {
            throw new IllegalArgumentException("That combination of buffers, offsets and length to xor result in out-of-bond accesses.");
        }
