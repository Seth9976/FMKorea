...
public final class zzfql {
    private static final zzfpx zza;

    static {
        zzfql.zza = new zzfpx("PhoneskyVerificationUtils");
    }
    ...
    label_3:
        for(int v = 0; v < arr_signature.length; ++v) {
            String s = zzfqk.zza(arr_signature[v].toByteArray());
            if("8P1sW0EPJcslw7UzRsiXL64w-O50Ed-RBICtay1g24M".equals(s) || (Build.TAGS.contains("dev-keys") || Build.TAGS.contains("test-keys")) && "GXWy8XF3vIml3_MfnmSmyuKBpT3B0dWbHRR_4cgq-gA".equals(s)) {
                return true;
            label_10:
                zzfql.zza.zzd("Phonesky package is not signed -- possibly self-built package. Could not verify.", new Object[0]);
                break;
            }
        }
        try {
